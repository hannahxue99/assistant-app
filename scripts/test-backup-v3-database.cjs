...
async function main() {
  ...
  const legacyEntry = {
    id: 'legacy-entry-1', rawText: '旧日记原文', kind: 'info', summary: '旧日记', dueAt: null,
    timePrecision: 'date', remindAt: null, topic: '生活', tags: [], persons: [],
    parseStatus: 'ok', parseSource: 'rule', correctedFrom: null, createdAt: 800,
    updatedAt: 800, revisionAt: 800, done: 0, doneAt: null, source: 'text',
  };
  sqlite.prepare(`INSERT INTO entries (
    id,raw_text,kind,summary,due_at,time_precision,remind_at,topic,tags,persons,parse_status,
    parse_source,corrected_from,created_at,updated_at,revision_at,done,done_at,source
  ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
    legacyEntry.id, legacyEntry.rawText, legacyEntry.kind, legacyEntry.summary, legacyEntry.dueAt,
    legacyEntry.timePrecision, legacyEntry.remindAt, legacyEntry.topic, JSON.stringify(legacyEntry.tags),
    JSON.stringify(legacyEntry.persons), legacyEntry.parseStatus, legacyEntry.parseSource,
    legacyEntry.correctedFrom, legacyEntry.createdAt, legacyEntry.updatedAt, legacyEntry.revisionAt,
    legacyEntry.done, legacyEntry.doneAt, legacyEntry.source,
  );
  sqlite.exec(`
    ...
    INSERT INTO conversation_segments VALUES ('legacy-history','历史记录','closed',800,800,800);
    ...
    INSERT INTO assistant_event_updates VALUES ('update-legacy','event-1','来自旧记录的真实进展',900,'legacy-legacy-entry-1','legacy:update',900,NULL);
    INSERT INTO assistant_object_relations VALUES ('relation-1','todo','todo-1','belongs_to','event','event-1','message-user-1',1400,NULL);
    INSERT INTO assistant_object_relations VALUES ('relation-legacy-source','todo','todo-1','related','event','event-1','legacy-legacy-entry-1',900,NULL);
    INSERT INTO assistant_object_relations VALUES ('relation-legacy-endpoint','message','legacy-legacy-entry-1','source','event','event-1','legacy-legacy-entry-1',900,NULL);
    ...
    INSERT INTO assistant_memory_sources VALUES ('memory-source-legacy','memory-1','legacy-legacy-entry-1','旧记录中的证据',900);
  `);
  ...
  assert.equal(envelope.payload.eventUpdates.length, 2);
  assert.equal(envelope.payload.eventUpdates.find(item => item.id === 'update-legacy').sourceMessageId, null);
  ...
  assert.ok(!markdown.includes('legacy-legacy-entry-1'));
