...
async function main() {
  ...
  // 回归：工具结果含敏感词被内容风控 400 拒绝时，中点化降级重试一次而不是整轮失败。
  const riskBodies: any[] = [];
  let riskFetchIndex = 0;
  const riskResult = await requestAssistantTurn({
    settings: {
      llmEnabled: true, llmBaseUrl: 'https://example.test/v1', llmKey: 'secret', llmModel: 'fixture-model',
    },
    context: { contextBlock: '上下文', recentMessages: [{ id: 'u', role: 'user', content: '十一出行', createdAt: 1 }] },
    fetchImpl: async (_url, init) => {
      const body = JSON.parse(String(init?.body));
      ...
      if (riskFetchIndex === 1) {
        return new Response(JSON.stringify({
          choices: [{ message: { content: '', reasoning_content: '先搜索事件。', tool_calls: [{ id: 'call-1', type: 'function', function: { name: 'search_events', arguments: '{"query": "十一出行"}' } }] }, finish_reason: 'tool_calls' }],
        }), { status: 200, headers: { 'content-type': 'application/json' } });
      }
      if (riskFetchIndex === 2) {
        return new Response(JSON.stringify({
      ...
      return new Response(JSON.stringify({
        choices: [{ message: { content: '{\"reply\":\"已找到十一出行事件。\",\"segment\":{\"action\":\"continue\"}}' }, finish_reason: 'stop' }],
      }), { status: 200, headers: { 'content-type': 'application/json' } });
    },
    executeReadTool: async call => ({
      toolCallId: call.id,
      name: 'search_events',
      result: { query: '十一出行', events: [{ id: 'e1', title: '十一出行', currentState: '10月3日哈尔滨飞北京返程机票', status: 'active', revision: 1, updatedAt: 1 }] },
      readEventIds: ['e1'],
      readTodoIds: [],
      readMemoryIds: [],
    }),
  });
  check(riskResult.reply.includes('十一出行'),
    '内容风控 400 后中点化降级重试应拿到模型回复，不得整轮失败');
  check(riskResult.providerMetadata.protocolWarnings.includes('content_risk_retried'),
    '内容风控降级必须记录协议警告');
  check(riskBodies[2].messages.some((message: any) => message.role === 'tool'
    && message.content.includes('北·京')),
    '重试请求的工具结果必须中点化敏感词');
  check(riskFetchIndex === 3, '降级重试恰好一次，不得无限循环');

  // 二次仍风控：允许失败，但错误码必须是 content-risk 便于上层与网络错误区分。
  let doubleRiskError: any = null;
  try {
    await requestAssistantTurn({
      settings: {
        llmEnabled: true, llmBaseUrl: 'https://example.test/v1', llmKey: 'secret', llmModel: 'fixture-model',
      ...
      fetchImpl: async (_url, init) => {
        ...
        return new Response(JSON.stringify({
          choices: [{ message: { content: '', reasoning_content: '搜索', tool_calls: [{ id: 'call-1', type: 'function', function: { name: 'search_events', arguments: '{}' } }] }, finish_reason: 'tool_calls' }],
        }), { status: 200, headers: { 'content-type': 'application/json' } });
      },
      executeReadTool: async call => ({
        toolCallId: call.id,
        name: 'search_events',
        result: { query: 'x', events: [] },
        readEventIds: [],
        readTodoIds: [],
        readMemoryIds: [],
      }),
    ...
    doubleRiskError = error;
  }
  check(doubleRiskError?.code === 'content-risk',
    '二次风控失败必须带 content-risk 错误码');
