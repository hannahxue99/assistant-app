...
1. 用户消息立即保存并显示。
2. 系统选择最近对话、相关历史段、事件、待办和长期记忆作为有界上下文。
3. 模型一次返回自然回复与结构化操作候选。
4. 本地校验协议、目标对象、日期、权限、记忆准入和幂等键。
5. 助手消息、有效操作、关系和决策日志在事务中保存。
6. 页面流式展示回复，并用一张紧凑回执显示真正完成的变化。
7. 用户可以撤销本轮已提交操作；停止未完成请求时不提交业务变化。
...
- 一条用户消息尽量只进行一次主模型调用；回复与操作候选共享结果。
...
当前进度以 2026-09-17 已合并并安装的 Release 为准。
...
| 旧数据迁移、导入导出 | 已实现 | 原声、待办、事件和记忆可迁移或备份 |
