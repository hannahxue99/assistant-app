...
export const ASSISTANT_PROMPT_VERSION = 'xiaozhi-actions-v10-on-demand-reads';
...
export function buildAssistantPromptMessages(input: {
  ...
  const system = [
    ...
    '- 你可以使用只读工具搜索和读取真实事件、待办。先判断回答或操作是否需要真实对象状态；不需要就不要调用工具。',
    '- 当前上下文若提供“已有有效快照”，且包含所需字段和 revision，直接复用，不要重复搜索或读取。',
    '- 搜索结果只用于发现候选，不代表已经读取完整状态。需要精确回答或更新已有对象时，先搜索，再按精确 ID 调用 get_event/get_todo；不要凭标题编造 ID。',
    '- 更新已有对象前必须获得完整详情：目标必须来自本轮 get_event/get_todo 或已有有效快照。只有 ID、自然语言历史、分段关联或搜索候选都不授权写入。',
