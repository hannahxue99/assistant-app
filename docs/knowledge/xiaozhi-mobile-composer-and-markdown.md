...
PR #31 建立了小知移动端输入区、历史位置、置底按钮和 Markdown 的基础交互；PR #32 继续解决了 Release 长历史下键盘与消息底部动画不同步的问题，并完成输入框、Tab 页面与对话字号的视觉收口。
...
- 三个 Tab 页面与底部导航均使用实体纯白，底部导航仅保留极淡暖灰顶分隔线，不使用灰底或模糊材质。
- 输入框保持独立悬浮卡片，用克制边框和暖灰阴影轻微外凸；用户发送后的消息与小知最终回复正文统一为 `16px / 23px`。
...
PR #32 在真实 Release 长历史下证明，仅在布局回调中校正 offset 仍会让正文、输入框和系统键盘出现时间差。最终使用 `react-native-reanimated` 的 `useAnimatedKeyboard()` 在 UI 线程读取键盘实时高度，并由同一份 `keyboardLift` 驱动：

- `following` 模式：正文与输入框同帧上移或回落，消息末端始终位于输入框上方。
- `history` 模式：正文位置不动，只有输入框与键盘同帧上移或回落。
- `keyboardLift` 由键盘高度减去输入框关闭态到屏幕底部的实测间隙得出，避免再用独立时长猜测系统动画。
...
- 需要与系统键盘同步的多个视图，必须共享 UI 线程的同一个键盘高度信号，不要为正文和输入框分别定时。
...
- PR #32 追加验收：默认末端与历史位置的键盘升降、正文/输入框/键盘同帧、下滑阴影、纯白 Tab 和底部导航、输入框外凸、16px 对话正文；用户于 2026-09-24 确认“整体验收都 OK”。
...
- PR #32 分支：`codex/xiaozhi-keyboard-bottom-anchor`。
- 发布前基线 / 整体回滚点：`6b48994dcc279b8f64abdbd9cf37876095e7c5a9`。
- PR #32 整体回滚点：`5e62118fa07abb3cedc435b9f12618109315fb75`。
