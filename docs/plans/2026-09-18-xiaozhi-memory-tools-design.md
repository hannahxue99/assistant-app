...
- 精确结果不符合预期：模型可继续调用其他搜索或读取工具，单轮最多 6 轮、12 次读取；超出后进入无工具收敛轮，模型基于已读信息回答并声明未核实部分。
- 精确读取后版本变化：本地版本校验拒绝写入，不能覆盖新状态。
- 只搜索未精确读取：对既有记忆的激活、替代、遗忘均拒绝。
- 创建全新记忆：不要求先调用读取工具，继续按长期记忆准入规则校验。
- 缺少 `evidence` 等协议字段：`forget_memory` 缺 evidence 时降级为单项拒绝（`missing_evidence`）进入执行回执，其余记忆增量仍按严格协议失败；不做二次 JSON 修复。提示词对 forget 的字段要求与协议校验保持一致。
