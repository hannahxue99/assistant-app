...
实现简单，但会重复发送上下文、增加延迟和 Token，且用户仍会看到长时间等待，不采用。
...
- 真正无法修复的回复仍显示“回复出了点问题 · 重试”，但决策日志会写明例如 `reply_execution_claim`、`invalid_date_protocol` 或 `malformed_json`，不再只有笼统错误码。
...
5. 正常格式不触发第二次请求；损坏格式最多自动修复一次，不形成无限重试。
