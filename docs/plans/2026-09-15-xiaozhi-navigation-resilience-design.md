...
```mermaid
flowchart TD
    H[首页] -->|首次点击或原本位于末端| F[小知获得焦点]
    F --> L[读取最新消息与回执]
    L --> S[内容完成布局]
    S --> E[首次进入或末端跟随时定位到最末端]
...
- 首次进入小知时显示最后一条消息的底部，输入框不会遮住回执。普通切回时，只有离开前已经位于末端才继续显示末端。
- 从某条消息的待办或事件进入关联页面再返回时，优先恢复离开前的阅读位置，不因消息刷新强制滚到底部。
...
8. 从历史消息或回执进入关联待办/事件再返回后，仍停留在离开前的消息附近；首次进入、发送消息和重试仍定位到最新回复。
