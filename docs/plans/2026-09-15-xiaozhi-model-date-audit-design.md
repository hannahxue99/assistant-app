...
真机日志确认，“下个月11号还款10万”曾被模型只输出为 `update_event`，协议、本地校验和事务均未拒绝任何待办；根因是模型把未来行动误当成了事件状态描述。

采用提示词判断准则与固定质量用例共同校正，不增加本地关键词规则：

- 用户明确表示自己将在未来时间执行一个具体动作时，即使使用陈述句而非“提醒我”，也应判断为待办。
- 若该动作属于已有持续事件，应同时更新事件、创建待办并建立关联；事件状态更新不能替代可执行待办。
- 仅描述可能性、外部变化、他人动作、预测，或尚未接受的建议时，不建立待办。

三种方案比较：

1. **强化同一次主模型的判断准则与正反例（采用）**：保持一次调用和模型语义判断，延迟与 Token 成本最低。
2. **再调用一个模型复核是否遗漏待办（不采用）**：可增加召回，但每轮增加延迟、Token 和两次判断不一致的处理成本。
3. **本地看到未来日期就补待办（不采用）**：无法区分承诺、预测和假设，且违背“由模型理解语义”的既定设计。

固定正例：在“公积金贷款还款”事件上下文中，参考时间为 2026-09-15，用户说“下个月11号还款10万”，候选操作应包含 `update_event`、日期为 2026-10-11 的 `create_todo`、以及 `link_todo_event`。固定反例：“银行说下个月可能调整利率”可以更新事件，但不能因此建立用户待办。
