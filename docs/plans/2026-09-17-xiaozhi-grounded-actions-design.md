...
数量上限防失控（成本、上下文、注意力），不承担安全职责——安全由逐项本地校验保证。上限：operations 10、event_deltas 4、delta 内 todos/progress 各 10、memory_deltas 2。模型超限不再整轮失败：解析层截断取前 N 条继续，丢弃数进入执行回执（truncated 统计），最终回复如实告知未处理部分及如何补做，同时记录 `*_truncated` 协议警告供观测截断频率。结构性错误（缺字段、非法枚举、非法日期）仍整轮拒绝。

### 1. 模型按需读取

提供九个只读工具（搜索/读取/列表各三类对象）：

- `search_events(query)`：搜索活跃事件，返回精简候选（前 8 条）。
...
- `list_events(pinned_only)`：列出全部活跃事件 ID+标题（紧凑总览，不含正文）。
- `list_todos(include_done)`：列出待办 ID+摘要。
- `list_memories(status)`：列出记忆 ID+类别+内容。

列表工具服务“列出全部”“有哪些”类总览需求：搜索有 8 条截断盲区，列表一次拿全量；列表结果不含正文、不授权写入，精确操作仍需 get。

工具阶段最多 6 轮、12 次调用，搜索结果每次最多 8 条；超出后不判失败，进入一次无工具收敛轮，模型基于已读信息给出计划并声明未核实部分。基础上下文不提供事件/待办候选正文；搜索与读取工具是模型获得现有对象真实状态的唯一入口。
...
- 工具结果中给模型的时间字段一律序列化为 ISO 日期字符串，不得输出毫秒时间戳：原始时间戳的 13 位数字串会与用户内容中的敏感词组合触发上游内容风控（实测：currentState 含"北京"且事件 updatedAt 含特定数字组合时，续轮请求 400 Content Exists Risk）。ISO 表示对模型语义无损，并从表示层消除整类组合巧合。revision 保持数字（本地写入校验必需的小整数）。
...
6. 运行状态逐行累加：每个完成的阶段定格为一行带耗时，当前阶段持续转圈计时；计时持续递增。落定后收成单行过程摘要（如“读取 3 秒 · 思考 8 秒 · 更新 1 秒”），各阶段实际耗时持久化到助手消息；纯聊天轮退化为“思考了 X 秒”。
